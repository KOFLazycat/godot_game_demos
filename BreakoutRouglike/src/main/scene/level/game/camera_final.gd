...
#			print("Max dist: ", max_dist)
	...
	while _last_shook_timer >= _period_in_ms:
		...
		#var new_xx = randf_range(-1.0, 1.0)
		var new_x: float = noise.get_noise_2d(noise.seed, _noise_y)
		var x_component = intensity * (_previous_x + (delta * (new_x - _previous_x)))
		#var new_yy = randf_range(-1.0, 1.0)
		var new_y: float = noise.get_noise_2d(noise.seed * 2, _noise_y)
		var y_component = intensity * (_previous_y + (delta * (new_y - _previous_y)))
		#print(Vector2(new_xx, new_yy), Vector2(new_x, new_y))
		_previous_x = new_x
		_previous_y = new_y
...
func start_tracking(new_target: Node2D) -> void:
