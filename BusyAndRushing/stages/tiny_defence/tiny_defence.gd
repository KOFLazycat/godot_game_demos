...
func on_timer_timeout() -> void:
	# left
	var tiny_defence_torch_left: CharacterBody2D = tiny_defence_torch_tscn.instantiate() as CharacterBody2D
	tiny_defence_torch_left.global_position = marker_2d_left.global_position + Vector2(randf_range(-20, 20), randf_range(-15, 15))
	tiny_defence_torch_left.scale = Vector2(0.5, 0.5)
	add_child(tiny_defence_torch_left)
	# right
	var tiny_defence_torch_right: CharacterBody2D = tiny_defence_torch_tscn.instantiate() as CharacterBody2D
	tiny_defence_torch_right.global_position = marker_2d_right.global_position + Vector2(randf_range(-20, 20), randf_range(-15, 15))
	tiny_defence_torch_right.scale = Vector2(0.5, 0.5)
	add_child(tiny_defence_torch_right)
